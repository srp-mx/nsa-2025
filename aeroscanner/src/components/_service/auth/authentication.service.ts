...
export class AuthenticationService {
  ...
  public login(credenciales: {username?: string, password?: string}): Observable<HttpResponse<LoginResponse>> {
    return this.http.post<LoginResponse>(`/auth/login`, credenciales, { observe: 'response' });
  }
  ...
  public refreshToken(): Observable<{access: string}> {
    ...
    return this.http.post<{access: string}>(`/auth/refresh/`, this.refresh_token );
  }

  public register(user: User): Observable<{message: string}> {
    return this.http.post<{message: string}>(`/auth/signup`, user);
  }

  public logout(): Observable<{message: string}> {
    return this.http.post<{message: string}>(`/auth/logout`, this.refresh_token);
  }
